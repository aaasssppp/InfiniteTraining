    ...
    class Program
    {
        ...
        static void BookTicket()
        {
            try
            {
                ...
                int availableSeats = Convert.ToInt32(train[availCol]);

                if (seats > availableSeats)
                {
                    Console.WriteLine("Only {0} seat(s) available in {1}.", availableSeats, cls);

                    // Ask user if they want to go on waiting list
                    Console.Write("Do you want to be added to the waiting list? (Y/N): ");
                    if (Console.ReadLine().ToUpper() == "Y")
                    {
                        for (int i = 0; i < seats; i++)
                        {
                            string wlQuery = @"INSERT INTO WaitingList (CustomerId, TrainNo, Class, DateOfTravel) 
                               VALUES (@cid, @tno, @class, @date)";
                            DBHelper.ExecuteNonQuery(wlQuery, new SqlParameter[]
                            {
                        ...
                        Console.WriteLine("Added to waiting list. You will be allotted a seat if any cancellation occurs.");
                        Console.ReadLine();
                        return;
                    }
                    else
        ...
        static void CancelTicket()
{
    try
    {
        ...
        string query = @"SELECT r.BookingId, t.TrainNo, t.TrainName, r.Class, r.SeatNo, r.TotalCost, r.DateOfTravel
                         FROM Reservation r
                         JOIN TrainDetails t ON r.TrainNo = t.TrainNo
        ...
        int trainNo = Convert.ToInt32(booking["TrainNo"]);
        string cls = booking["Class"].ToString();
        DateTime travelDate = Convert.ToDateTime(booking["DateOfTravel"]);
        ...
        // ====== WAITING LIST CHECK ======
        string wlQuery = @"SELECT TOP 1 WaitingId, CustomerId
                           FROM WaitingList
                           WHERE TrainNo = @tno AND Class = @class AND DateOfTravel = @date
                           ORDER BY WaitingId ASC"; // FIFO
        DataTable wl = DBHelper.ExecuteQuery(wlQuery, new SqlParameter[]
        {
            new SqlParameter("@tno", trainNo),
            new SqlParameter("@class", cls),
            new SqlParameter("@date", travelDate)
        ...
        if (wl.Rows.Count > 0)
        {
            int waitingId = Convert.ToInt32(wl.Rows[0]["WaitingId"]);
            int customerId = Convert.ToInt32(wl.Rows[0]["CustomerId"]);

            // Allot seat to waiting list user
            string insertBooking = @"INSERT INTO Reservation (CustomerId, TrainNo, DateOfTravel, Class, SeatNo, TotalCost)
                                     VALUES (@cid, @tno, @date, @class, 1, 
                                             (SELECT " + (cls == "1AC" ? "Class1Cost" : cls == "2AC" ? "Class2Cost" : "Class3Cost") + @" FROM TrainDetails WHERE TrainNo = @tno))";
            DBHelper.ExecuteNonQuery(insertBooking, new SqlParameter[]
            {
            ...
            // Reduce availability again
            string reduceSeats = $@"UPDATE TrainDetails
                                    SET {availCol} = {availCol} - 1
                                    WHERE TrainNo = @tno";
            DBHelper.ExecuteNonQuery(reduceSeats, new SqlParameter[]
            {
                new SqlParameter("@tno", trainNo)
            });

            // Remove from waiting list
            string deleteWL = "DELETE FROM WaitingList WHERE WaitingId = @wid";
            DBHelper.ExecuteNonQuery(deleteWL, new SqlParameter[]
            {
                new SqlParameter("@wid", waitingId)
            });

            Console.WriteLine($"Seat allotted to waiting list customer (UserId: {customerId}).");
        }
